...
if CTLDCost==true then -- this portion is if stuff cost money and CTLDCost is true which is defualt.
    ...
    Foothold_ctld:AddCratesCargo(pricedName("IRIS T SLM LN (1cr)"), {"CTLD_CARGO_IRISTSLM-LN"}, CTLD_CARGO.Enum.FOB, 4, 3500, 5, "SAM/AAA")
    Foothold_ctld:AddCratesCargo(pricedName("IRIS T SLM C2 (1cr)"), {"CTLD_CARGO_IRISTSLM_C2"}, CTLD_CARGO.Enum.FOB, 4, 2500, 5, "SAM/AAA")
...
Foothold_ctld:AddCratesCargo("IRIS T SLM LN (1cr)", {"CTLD_CARGO_IRISTSLM-LN"}, CTLD_CARGO.Enum.FOB, 4, 3500, 5, "SAM/AAA")
Foothold_ctld:AddCratesCargo("IRIS T SLM C2 (1cr)", {"CTLD_CARGO_IRISTSLM_C2"}, CTLD_CARGO.Enum.FOB, 4, 1900, 5, "SAM/AAA")
...
if lfs then
if Era == 'Coldwar' then  
FarpFileName = "Foothold_Germany_CTLD_FARPS_Coldwar.csv"
else
FarpFileName = "Foothold_Germany_CTLD_FARPS_Modern.csv"
end
function SaveFARPS()
...
if Era=='Coldwar' then
Foothold_ctld.filename = "Foothold_Germany_CTLD_Save_Coldwar.csv" -- example filename
else
Foothold_ctld.filename = "Foothold_Germany_CTLD_Save_Modern.csv" -- example filename
end
...
function resetSaveFileAndFarp()
  ...
  -- 3) Also reset your in-memory FARPs table if you like:
...
local function updateLastPickupZone(Group, ReferenceUnit)
    ...
    if coord and coord.GetVec3 then
        local ok, vec = pcall(function() return coord:GetVec3() end)
        if ok and vec then
        ...
        elseif currentZone.side == 2 then
            if data.pickupZoneName and data.zoneName and data.pickupZoneName == data.zoneName then
                ...
                if CTLDCost and priceOf then
                    local gid = data.deployer and data.deployer.GetDCSObject and data.deployer:GetDCSObject() and data.deployer:GetDCSObject():getID() or nil
                    local refund = priceOf(cname) or 0
                    if refund > 0 then bc:credit(2, refund, gid, cname) end
                end
                troopGroup:Destroy()
                cleanupDeployment(troopGroupName)
                scheduleNext(1)
