...
function GetValidCords(zoneName, allowed, attempts)
	...
	for _=1,attempts do
		...
		if coord then
			...
			if string.find(zoneName, "Luneburg") and (not allowed or allowed[st]) then return coord end
	...
	if zoneName then
		...
		if string.find(zoneName, "Luneburg") then
		allowed = isCarrierZone and {
		[land.SurfaceType.WATER] = true,
		[land.SurfaceType.SHALLOW_WATER] = true
		} or {
		[land.SurfaceType.LAND] = true,
...
function Frontline.PickStationNearZone(zoneName, mySide, offsetNmTowardFriendly, lateralJitterNm, forwardJitterNm, minBufferNm, avoidEnemyMinNm)
    ...
    if sc and sp and ((sc>=0) ~= (sp>=0)) then
        local push = ((minBufferNm or avoidEnemyMinNm or 10) * NM)
        st = vadd(st, vmul(f, push))
    end
...
function GroupCommander:_assignHeloRoute(grName, zoneName)
	env.info("GroupCommander:_assignHeloRoute "..tostring(grName).." to "..tostring(zoneName))
					...
					if set and #set > 0 then
						...
						if SpawnType and SpawnType.kind == 'parking' and SpawnType.airbase and SpawnType.spots and #SpawnType.spots>0 then
							...
							if tpl then
								...
								if not spawned then self.state = 'inhangar'; self.lastStateTime = timer.getAbsTime(); return end
