...
do -- the main scope
	...
	local function groupSpawned(event)
		...
		if event.id == world.event.S_EVENT_BIRTH and timer.getTime() > event.time + 0.01 then
...
do -- group functions scope
	...
	function mist.teleportToPoint(vars) -- main teleport function that all of teleport/respawn functions call
		...
		if point and radius >= 0 then
			...
			if valid == false then
				log:error('Point supplied in variable table is not a valid coordinate. group=$1 zone=$2 valid=$3',
          gpName or 'nil', vars.spawnzone or 'nil', validTerrain)
				return false
			end
		end
